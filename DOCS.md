...
- Postmates (Web API)
  - Both Gruhub and Doordash set location data using latitude and longitude values passed in via the `location` object in the JSON formatted body. However, Postmates does not use that data to set location. Instead, Postmates uses a cookie that must be set in the browser prior to making the API call. This cookie can be retried using the `/set/location` endpoint.
